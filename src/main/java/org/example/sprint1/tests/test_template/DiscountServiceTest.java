...
/*
getDisplayName(int invocationIndex) — задаёт уникальное имя для каждого запуска теста. Это имя отображается в отчётах,
что помогает быстро понять, какие параметры были использованы.
supportsParameter(ParameterContext parameterContext, ExtensionContext extensionContext) — метод интерфейса ParameterResolver,
который проверяет, поддерживается ли данный параметр. Мы указываем, что первые три параметра являются String, а четвёртый — double.
resolveParameter(ParameterContext parameterContext, ExtensionContext extensionContext) — возвращает значения параметров.
Метод подбирает нужное значение в зависимости от индекса параметра в тестовом методе.
getAdditionalExtensions() — возвращает текущий контекст как ParameterResolver, чтобы JUnit знал,
что контекст можно использовать для разрешения параметров тестового метода.
*/
